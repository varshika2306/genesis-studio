...
export const Route = createFileRoute("/signup")({
  head: () => ({ meta: [{ title: "Create account — AI Content Studio" }] }),
...
function SignupPage() {
  return (
    <AuthShell
      ...
      subtitle="Spin up your AI creator OS in 30 seconds."
      footer={<>Already have an account? <Link to="/login" className="text-cyan-glow hover:underline">Sign in</Link></>}
    >
      <form className="space-y-3" onSubmit={(e) => e.preventDefault()}>
        <Field label="Email" type="email" placeholder="you@studio.com" />
        <Field label="Password" type="password" placeholder="Choose a strong one" />
        <Link to="/onboarding">
